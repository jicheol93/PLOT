...
def build_transforms(img_size=(384, 128), aug=False, is_train=True):
    ...
    if aug:
        transform = T.Compose([
            ...
            ##T.RandomErasing(scale=(0.1, 0.5), value=mean),
        ])
    else:
        transform = T.Compose([
            T.Resize((height, width)),
...
def build_transforms_woflip(img_size=(384, 128), aug=False, is_train=True):
    height, width = img_size
...
def build_dataloader(args, tranforms=None):
    ...
    if args.training:
        train_transforms = build_transforms(img_size=args.img_size,
                                            aug=args.img_aug,
                                            is_train=True)
        train_transforms_woflip = build_transforms_woflip(img_size=args.img_size,
                                            aug=args.img_aug,
                                            is_train=True)
