...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, num_layers=None):
        super().__init__()
        self.width = width
        self.layers = layers
    ...
    def forward(self, x: torch.Tensor):
        ...
        return self.resblocks[:self.num_layers](x), None
...
class Transformer_t(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        for block in self.resblocks:
            ...
            weights_all_blocks.append(weight)
            """
            x, x_m, ln_2, weight = block(x)
            weights_all_blocks.append(weight)
            x_m_all_blocks.append(x_m)
            ln_2_all_blocks.append(ln_2)
        x = ln_2_all_blocks[-2](x_m_all_blocks[-2])
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: Tuple[int, int], patch_size: int, stride_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        ##self.transformer = Transformer(width, layers, heads, num_layers=-1)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image):
        ...
        x, weights =  self.visual(image.type(self.dtype))
        return x, weights
    ...
    def forward(self, image, text):
        image_features, w_i = self.encode_image(image)
        text_features, w_t = self.encode_text(text)
        ...
        return image_features, w_i, text_features, w_t
...
def convert_weights(model: nn.Module):
    ...
    def _convert_weights_to_fp16(l):
        ...
        if isinstance(l, (nn.GRUCell)):
            l.weight_ih.data = l.weight_ih.data.half()
            l.weight_hh.data = l.weight_hh.data.half()
            if l.bias is not None:
                ##pdb.set_trace()
                l.bias_ih.data = l.bias_ih.data.half()
                l.bias_hh.data = l.bias_hh.data.half()
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            ##pdb.set_trace()
            x_tmp , attn = self._mha_block(self.norm2(x), memory, memory_mask, memory_key_padding_mask, memory_is_causal)
            ##x = x + self._mha_block(self.norm2(x), memory, memory_mask, memory_key_padding_mask, memory_is_causal)
            x = x + x_tmp
            x = x + self._ff_block(self.norm3(x))
        else:
            x = self.norm1(x + self._sa_block(x, tgt_mask, tgt_key_padding_mask, tgt_is_causal))
            x_tmp, attn = self._mha_block(x, memory, memory_mask, memory_key_padding_mask, memory_is_causal)
